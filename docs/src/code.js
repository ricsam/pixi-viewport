...
let _fps,
  ...
  domEase;
function viewport() {
  _viewport = _application.stage.addChild(
    new Viewport({
...
function border() {
  ...
  line
    .stroke(10, 0xff0000)
    .rect(0, 0, _viewport.worldWidth, _viewport.worldHeight);
}
...
function stars() {
  const stars =
    ((_viewport.worldWidth * _viewport.worldHeight) / Math.pow(STAR_SIZE, 2)) *
    0.1;
  ...
  for (let i = 0; i < stars; i++) {
    ...
    do {
      x = Random.range(
        STAR_SIZE / 2 + BORDER,
        _viewport.worldWidth - STAR_SIZE - BORDER,
      );
...
function createTarget() {
  const x = Random.range(
    OBJECT_SIZE / 2 + BORDER,
    _viewport.worldWidth - OBJECT_SIZE / 2 - BORDER,
  );
  const y = Random.range(
    OBJECT_SIZE / 2 + BORDER,
    _viewport.worldHeight - OBJECT_SIZE / 2 - BORDER,
  );
  const target = ease.target(_object, { x, y }, OBJECT_SPEED, {
    wait: Random.chance(0.75) ? Random.range(500, 3000) : null,
  });
...
function object() {
  ...
  ease.add(
    _object,
    { rotation: Math.PI * 2 },
    { duration: OBJECT_ROTATION_TIME, repeat: true, ease: "linear" },
  );
  createTarget();
}
...
function click(data) {
  for (let star of _stars) {
    if (
      Math.abs(data.world.x - star.position.x) +
        Math.abs(data.world.y - star.position.y) <
      STAR_SIZE * 2
    ) {
      console.log("star match");
      ease.add(
        star,
        { width: STAR_SIZE * 3, height: STAR_SIZE * 3 },
        { reverse: true },
      );
      //return
    }
