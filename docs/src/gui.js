...
function guiDrag() {
  _gui.add(options, "drag").onChange(function (value) {
    if (value) {
...
function guiClamp() {
  function change() {
    _viewport.clamp({
      direction:
        options.clamp.x && options.clamp.y
          ? "all"
          : options.clamp.x
            ? "x"
            : "y",
      underflow: options.clamp.underflow,
    });
  }
  ...
  clamp.add(options.clamp, "clamp").onChange(function (value) {
    if (value) {
      change();
      add();
    } else {
...
function guiPinch() {
  function change() {
    const center =
      options.pinch.centerX || options.pinch.centerY
        ? { x: options.pinch.centerX, y: options.pinch.centerY }
        : null;
    _viewport.pinch({
      noDrag: options.pinch.noDrag,
      center,
      percent: options.pinch.percent,
    });
  }
  ...
  pinch.add(options.pinch, "pinch").onChange(function (value) {
    if (value) {
      change();
      add();
    } else {
...
function guiBounce() {
  function change() {
    _viewport.bounce({
      sides: options.bounce.sides,
      time: options.bounce.time,
      ease: options.bounce.ease,
      friction: options.bounce.friction,
      underflow: options.bounce.underflow,
    });
  }
  ...
  function add() {
    ...
    time = bounce
      .add(options.bounce, "time", 0, 2000)
      .step(50)
      .onChange(change);
  ...
  bounce.add(options.bounce, "bounce").onChange(function (value) {
    if (value) {
      change();
      if (!time) {
        add();
...
function guiDecelerate() {
  function change() {
    _viewport.decelerate({
      friction: options.decelerate.friction,
      minSpeed: options.decelerate.minSpeed,
    });
  }
  ...
  decelerate.add(options.decelerate, "decelerate").onChange(function (value) {
    if (value) {
      change();
      if (!friction) {
        add();
...
function guiSnap() {
  function change() {
    _viewport.snap(options.snap.x, options.snap.y, {
      interrupt: options.snap.interrupt,
      time: options.snap.time,
      ease: options.snap.ease,
      friction: options.snap.friction,
      topLeft: options.snap.topLeft,
    });
  }
  ...
  snap.add(options.snap, "snap").onChange(function (value) {
    if (value) {
      change();
      add();
    } else {
...
function guiFollow(target) {
  function change() {
    _viewport.follow(target, {
      speed: options.follow.speed,
      radius: options.follow.radius,
      acceleration: options.follow.acceleration,
    });
  }
  ...
  follow.add(options.follow, "follow").onChange(function (value) {
    if (value) {
      change();
      add();
    } else {
...
function guiWheel() {
  function change() {
    const center =
      options.wheel.centerX || options.wheel.centerY
        ? { x: options.wheel.centerX, y: options.wheel.centerY }
        : null;
    _viewport.wheel({
      percent: options.wheel.percent,
      minWidth: options.wheel.minWidth,
      maxWidth: options.wheel.maxWidth,
      minHeight: options.wheel.minHeight,
      maxHeight: options.wheel.maxHeight,
      center,
      lineHeight: options.wheel.lineHeight,
    });
  }
  ...
  wheel.add(options.wheel, "wheel").onChange(function (value) {
    if (value) {
      change();
      add();
    } else {
...
function guiClampZoom() {
  function change() {
    _viewport.clampZoom({
      minWidth: options.clampZoom.minWidth,
      maxWidth: options.clampZoom.maxWidth,
      minHeight: options.clampZoom.minHeight,
      maxHeight: options.clampZoom.maxHeight,
    });
  }
  ...
  clampZoom.add(options.clampZoom, "clampZoom").onChange(function (value) {
    if (value) {
      change();
      add();
    } else {
...
function guiSnapZoom() {
  function change() {
    options.snapZoom.center =
      options.snapZoom.centerX || options.snapZoom.centerY
        ? { x: options.snapZoom.centerX, y: options.snapZoom.centerY }
        : null;
  ...
  function add() {
    ...
    removeOnComplete = snapZoom
      .add(options.snapZoom, "removeOnComplete")
      .onChange(change);
  ...
  snapZoom.add(options.snapZoom, "snapZoom").onChange(function (value) {
    if (value) {
      change();
      add();
    } else {
...
function guiMouseEdges() {
  function change() {
    ...
    _viewport.mouseEdges({
      radius: me.radius !== 0 ? me.radius : null,
      distance: me.distance !== 0 ? me.distance : null,
      top: me.top !== 0 ? me.top : null,
      bottom: me.bottom !== 0 ? me.bottom : null,
      left: me.left !== 0 ? me.left : null,
      right: me.right !== 0 ? me.right : null,
      speed: me.speed,
      reverse: me.reverse,
      noDecelerate: me.noDecelerate,
      linear: me.linear,
    });
  }
  ...
  function add() {
    ...
    noDecelerate = mouseEdges
      .add(options.mouseEdges, "noDecelerate")
      .onChange(change);
    linear = mouseEdges.add(options.mouseEdges, "linear").onChange(change);
  }

  const mouseEdges = _gui.addFolder("mouseEdges");
  mouseEdges.add(options.mouseEdges, "mouseEdges").onChange(function (value) {
    if (value) {
      change();
      add();
    } else {
