...
export class Viewport extends Container {
  ...
  constructor(options: IViewportOptions) {
    ...
    if (this.options.disableOnContextMenu) {
      this.options.events.domElement.addEventListener(
        "contextmenu",
  ...
  destroy(options?: DestroyOptions): void {
    ...
    if (this.options.disableOnContextMenu) {
      this.options.events.domElement?.removeEventListener(
        "contextmenu",
        this._disableOnContextMenu,
      );
    }
  ...
  resize(
    ...
    screenHeight: number = typeof window === "undefined"
      ? 0
      : window.innerHeight,
    worldWidth?: number,
    worldHeight?: number,
   ...
   * @returns {PIXI.Point}
   */
  public toWorld<P extends PointData = Point>(
    x: number | PointData,
    y?: number,
  ): P {
   ...
   * @returns {PIXI.Point}
   */
  public toScreen<P extends PointData = Point>(
    x: number | PointData,
    y?: number,
  ): P {
    if (arguments.length === 2) {
